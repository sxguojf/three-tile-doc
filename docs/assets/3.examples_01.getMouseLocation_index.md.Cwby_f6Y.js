import{p as t,C as r,c as p,o,j as e,a4 as h,G as i,a2 as d,a as c,a5 as k,k as n,w as m}from"./chunks/framework.xbTv8SNN.js";import{I as u,$ as g}from"./chunks/index.Dw17pxVn.js";const b=`<style>\r
    html,\r
    body {\r
        color: white;\r
...
\r
    /**\r
     * 显示鼠标位置的经纬度和海拔高度\r
     * @param viewer 三维查看器\r
     * @param map 地图实例\r
     * @param id 显示信息的元素选择器（如 "#local-info"）\r
     */\r
...
<\/script>\r
`,E=JSON.parse('{"title":"1. 取得鼠标处经纬度海拔高度","description":"","frontmatter":{},"headers":[],"relativePath":"3.examples/01.getMouseLocation/index.md","filePath":"3.examples/01.getMouseLocation/index.md","lastUpdated":1773581106000}'),y={name:"3.examples/01.getMouseLocation/index.md"},_=Object.assign(y,{setup(A){const a=t(!0);return(D,s)=>{const l=r("ClientOnly");return o(),p("div",null,[s[1]||(s[1]=e("h1",{id:"_1-取得鼠标处经纬度海拔高度",tabindex:"-1"},[c("1. 取得鼠标处经纬度海拔高度 "),e("a",{class:"header-anchor",href:"#_1-取得鼠标处经纬度海拔高度","aria-label":'Permalink to "1. 取得鼠标处经纬度海拔高度"'},"​")],-1)),h(i(n(u),null,null,512),[[k,a.value]]),i(l,null,{default:m(()=>[i(n(g),{title:"Example",description:"地图下方显示鼠标所指经纬度、海拔高度信息",locale:"",select:"vue",order:"vue,react,html",github:"",gitlab:"",theme:"",lightTheme:"",darkTheme:"",stackblitz:"%7B%22show%22%3Atrue%7D",codesandbox:"%7B%22show%22%3Atrue%7D",codeplayer:"%7B%22show%22%3Afalse%7D",files:"%7B%22vue%22%3A%7B%7D%2C%22react%22%3A%7B%7D%2C%22html%22%3A%7B%7D%7D",scope:"",htmlWriteWay:"write",background:"undefined",visible:!0,onMount:s[0]||(s[0]=()=>{a.value=!1}),htmlCode:n(b)},null,8,["htmlCode"])]),_:1}),s[2]||(s[2]=d(`<p>实时显示鼠标处经纬度海拔高度信息是地图软件最常用的功能，这里使用了 TileMap 的根据屏幕坐标采用地面信息的方法。</p><p>three-tile 提供多种取得地面信息的方法，包括通过屏幕坐标、经纬度、世界坐标取得地面信息（地形采样）。</p><div class="tip custom-block"><p class="custom-block-title">TIP</p><p>three-tile 插件 plugin.getLocalFromMouse 辅助函数，简化取得鼠标位置信息</p></div><h2 id="_1-根据屏幕坐标取得地面信息" tabindex="-1">1. 根据屏幕坐标取得地面信息 <a class="header-anchor" href="#_1-根据屏幕坐标取得地面信息" aria-label="Permalink to &quot;1. 根据屏幕坐标取得地面信息&quot;">​</a></h2><p>通过 TileMap.getLocalInfoFromScreen()屏幕坐标处的地图经纬度以及海拔高度，鼠标在地图上移动时，取得屏幕坐标再调用它取得地面信息。函数的定义如下：</p><div class="language-ts vp-adaptive-theme line-numbers-mode"><button title="Copy Code" class="copy"></button><span class="lang">ts</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span style="--shiki-light:#6A737D;--shiki-dark:#6A737D;">/**</span></span>
...
<span class="line"><span style="--shiki-light:#6F42C1;--shiki-dark:#B392F0;">getLocalInfoFromScreen</span><span style="--shiki-light:#24292E;--shiki-dark:#E1E4E8;">(camera: Camera, pointer: Vector2): LocationInfo </span><span style="--shiki-light:#D73A49;--shiki-dark:#F97583;">|</span><span style="--shiki-light:#005CC5;--shiki-dark:#79B8FF;"> undefined</span><span style="--shiki-light:#24292E;--shiki-dark:#E1E4E8;">;</span></span></code></pre><div class="line-numbers-wrapper" aria-hidden="true"><span class="line-number">1</span><br><span class="line-number">2</span><br><span class="line-number">3</span><br><span class="line-number">4</span><br><span class="line-number">5</span><br><span class="line-number">6</span><br><span class="line-number">7</span><br></div></div><p>返回类型为 LocationInfo，类型定义如下：</p><ul><li>distance —— 射线投射原点和相交部分之间的距离。</li><li>point —— 相交部分的点（世界坐标）</li><li>face —— 相交的面</li><li>faceIndex —— 相交的面的索引</li><li>object —— 相交的物体</li><li>uv —— 相交部分的点的 UV 坐标。</li><li>uv1 —— 相交部分的点的第二组 UV 坐标</li><li>normal - 交点处的内插法向量</li><li>instanceId – 与 InstancedMesh 物体相交时的 instance 索引</li><li><mark> location: Vector3 —— 经纬度信息（经度、纬度、高度[米]）</mark></li></ul><p>最后一项 location 即为经纬度海拔高度信息，其中 x 为经度、y 为纬度、z 为海拔高度(m)。</p><h2 id="_2-根据经纬度取得地面信息" tabindex="-1">2. 根据经纬度取得地面信息 <a class="header-anchor" href="#_2-根据经纬度取得地面信息" aria-label="Permalink to &quot;2. 根据经纬度取得地面信息&quot;">​</a></h2><p>类似的，TileMap.getLocalInfoFromGeo()根据经纬度取得地面信息：</p><div class="language-ts vp-adaptive-theme line-numbers-mode"><button title="Copy Code" class="copy"></button><span class="lang">ts</span><pre class="shiki shiki-themes github-light github-dark vp-code" tabindex="0"><code><span class="line"><span style="--shiki-light:#6A737D;--shiki-dark:#6A737D;"> /**</span></span>
<span class="line"><span style="--shiki-light:#6A737D;--shiki-dark:#6A737D;"> * Get the ground information from latitude and longitude</span></span>
...
<span class="line"><span style="--shiki-light:#6A737D;--shiki-dark:#6A737D;"> * Get location information from world position</span></span>
