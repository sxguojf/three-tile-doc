...
three-tile 是一个开源的轻量级前端三维瓦片框架，基于 threejs 使用 typescript 开发，提供一个三维地形模型，能轻松给你的应用增加三维瓦片地图。经过数十版迭代，目前框架功能、效果能够满足生产环境使用，已在多个项目中得到了验证和应用。
...
- 依赖少：整个框架仅有 threejs（>=R171）一个依赖。
- 效果好：虽然很小巧，但地图显示效果不输 cesium 等重量级框架。
