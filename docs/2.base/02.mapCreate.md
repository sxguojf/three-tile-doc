...
瓦片地图类 TileMap 是地图的重点，它继承于 threejs 的 Mesh 类，对地图的绝大数操作均可通过对 TileMap 类完成。创建一个地图模型实例，并将它加入三维场景：
...
// 摄像机经纬度高度（km）转为世界坐标
const cameraPosition = map.geo2world(new THREE.Vector3(116.39199596764485, 39.91047669278009, 0.9));
// 调整摄像机位置
