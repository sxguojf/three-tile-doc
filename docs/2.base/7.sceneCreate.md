...
three-tile 基于 threejs 开发，但它并未接管 threejs 的三维场景创建，如 scene、renderer、camera、controls 等，只提供了地图模型。三维场景需要用户自己创建。

但为了方便使用，three-tile 提供了一个`GLVierwer`类用于快速创建三维场景。
...
|       `camera`        | `PerspectiveCamera` |                相机对象                 |
|      `controls`       |    `MapControls`    |               控制器对象                |
|      `ambLight`       |   `AmbientLight`    |               环境光对象                |
|      `dirLight`       | `DirectionalLight`  |               直射光对象                |
|      `container`      |    `HTMLElement`    |                容器元素                 |
| `fogFactor` (get/set) |      `number`       |               雾浓度因子                |
|     `width` (get)     |      `number`       |               容器的宽度                |
|    `height` (get)     |      `number`       |               容器的高度                |
|     `constructor`     |      `method`       |                构造函数                 |
|      `resize()`       |   `public method`   |                调整大小                 |
|      `animate()`      |  `private method`   |                动画循环                 |
|       `update`        |       `event`       | 更新事件，附带 `delta` 属性表示时间间隔 |

该类封装了threejs的常规初始化过程，提供 `Scene` 、`WebGLRenderer` 渲染器、`PerspectiveCamera` 相机和 `MapControls` 控制器、环境光、直射光等对象，并提供了调整大小和动画循环的方法。