...
第一人称游戏的以玩家视角渲染地图，模拟玩家看到的真实世界，与常规地图相比，仅是使用的控制器即不同。

为了方便用户使用，three-tile 对第一人称场景进行了封装：`PLViewer`，使用`PLViewer`初始化场景即可。仅需数十行代码即可完成一个简单的第一人称射击游戏。

`three-tile` 的地图不依赖 `camera`、`controls`，所以可以任意更换控制器，第一人称视角，只需要将控制器更换为 `threejs` 的 `FirstPersonControls`：
