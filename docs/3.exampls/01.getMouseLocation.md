...
下面简单讲下原理，以及根据其它坐标取得地面经纬度海拔高度的方法：
  ...
  viewer.container.addEventListener("pointermove", (evt) => {
    ...
    if (info) {
      ...
      el.innerHTML = `经度:${info.location.x.toFixed(
        6
      )}° 纬度:${info.location.y.toFixed(6)}° 海拔:${info.location.z.toFixed(
        6
      )}km`;
    }
  });
...
::: tip
这几个函数是通过射线法计算射线与瓦片交点取得，所以需要等瓦片加载完毕后才能计算，否则会返回空值。
:::
