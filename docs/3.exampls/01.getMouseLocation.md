...
:::tip three-tile 插件内置辅助函数，简化取得鼠标位置信息
...
function showLocation(viewer, map, id) {
  ...
  viewer.container.addEventListener("pointermove", (evt) => {
    ...
    if (lonlat) {
      ...
      el.innerHTML = `经度:${lonlat.x.toFixed(6)}° 纬度:${lonlat.y.toFixed(6)}° 海拔:${lonlat.z.toFixed(6)}km`;
    }
  });
}
...
## three-tile 提供多种取得地面信息的方法，包括通过屏幕坐标、经纬度、世界坐标取得地面信息等信息：
...
- <mark> location: Verctor3 —— 经纬度信息（经度、纬度、高度）</mark>

最后一项 location 即为经纬度海拔高度信息，其中 x 为经度、y 为纬度、z 为海拔高度(KM)。
...
function showLocation(viewer, map, id) {
  ...
  viewer.container.addEventListener("pointermove", (evt) => {
    ...
    if (info) {
      const el = document.querySelector(id);
      el.innerHTML = `经度:${info.location.x.toFixed(6)}° 纬度:${info.location.y.toFixed(6)}° 海拔:${info.location.z.toFixed(6)}km`;
    }
  });
}
showLocation(viewer, map, "#local-info");
